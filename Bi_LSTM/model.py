...
class Seq2SeqModel(object):
    def __init__(self, encoder_size, decoder_size, hidden_dim, input_dim, output_dim):
        ...
        encoder_cell_fw = tf.contrib.rnn.BasicLSTMCell(hidden_dim)
        encoder_cell_bw = tf.contrib.rnn.BasicLSTMCell(hidden_dim)
        ...
        encoder_final_state_c = tf.concat((state_fw.c, state_bw.c), 1)
        encoder_final_state_h = tf.concat((state_fw.h, state_bw.h), 1)

        state = tf.contrib.rnn.LSTMStateTuple(c = encoder_final_state_c, h = encoder_final_state_h)

        # construct lstm basic cell
